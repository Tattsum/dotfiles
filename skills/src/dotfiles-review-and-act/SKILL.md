...
- **自分の過去投稿の同定**: `mine == true` のスレッドを自分の過去指摘とみなす。判別は author が自分であること。`badged == false`（バッジ無しの自己投稿）でも、同じ `path` に自分のコメントがあれば**同一論点候補として扱い、新規投稿しない**（観点が復元できず厳密照合できないため、安全側に倒して重複を出さない）。
- **loose 一致**: 「同じ `path` × 同じ観点（focus 見出し）」で既存スレッドと一致するかを見る。`path:line` は使わない（`is_outdated == true` のスレッドは `line` が消え、行ベースの照合は再レビューでは必ず空振りする）。同一性の最終判断は機械化せず、`body_head` と今回の指摘内容を突き合わせてあなた自身が行う。
...
- **解消（確認済み）**: 前回指摘のファイルが今回のレビュースコープに含まれ、かつ再検出されなかった → 直ったとみなす。該当スレッドが未解決なら `first_comment_id` に「解消を確認」と**返信**する（スレッドの resolve はしない。スコープ内に居ても指摘箇所を必ず読んだ保証まではないため、settled 化は人間に委ねる）。
- **未再検出（今回スコープ外・要確認）**: 前回指摘のファイルが今回の変更ファイル一覧に無い → 今回は見ていない。返信も削除もせず、会話に「要確認」として残す。

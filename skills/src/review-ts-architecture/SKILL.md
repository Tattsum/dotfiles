...
# /review-ts-architecture

Review TypeScript / Vue / React frontend changes against `<base>...HEAD` using two independent architecture-focused subagents. This is review-only: do not modify files.
...
In scope:
- component responsibility and Container/Presentational separation
- Props size, prop drilling, and reusable UI/logic extraction into composables/hooks
...
```text
あなたは /review-ts-architecture コマンドの1名のレビュー担当です。レビューのみ行い、ファイルは編集しないでください。
...
If both subagents return no findings, say `コンポーネント責務・構成の観点では指摘はありません`.

...
license: MIT
argument-hint: "[write|score] [quick|full] [対象ファイルや依頼内容]"
---
...
Adapted from `coji/natural-japanese` (`skills/natural-japanese/`) at tag
`v1.5.0`, commit `21e632661a910bf97289c501089ad11eb8b4d85f`. MIT License
(see `LICENSE`). This copy is maintained independently and is not kept in
sync with upstream.

Local modifications from upstream:

- Removed `scripts/semantic.py` and the `exp` mode (`trust_remote_code=True`
  with an unpinned ~1GB model download).
- Removed `scripts/calibrate.py` (depends on upstream `corpus/`, not vendored).

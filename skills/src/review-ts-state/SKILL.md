...
Review TypeScript / Vue / React frontend changes against `<base>...HEAD` using two independent state-focused subagents. This is review-only: do not modify files.
...
In scope:
- state scope: local vs global (Redux/Zustand/Pinia/Vuex) and derived-state duplication
- effects and async: dependency arrays, cleanup, race conditions, and fetch error/loading handling
...
```text
あなたは /review-ts-state コマンドの1名のレビュー担当です。レビューのみ行い、ファイルは編集しないでください。
...
If both subagents return no findings, say `状態管理・副作用/非同期の観点では指摘はありません`.

...
# /review-ts-performance

Review TypeScript / Vue / React frontend changes against `<base>...HEAD` using two independent performance-focused subagents. This is review-only: do not modify files.
...
In scope:
- render performance: unnecessary re-renders, memoization, stable keys and props
- load/compute: heavy work on the main thread, code splitting/lazy loading, bundle size, list virtualization
...
```text
あなたは /review-ts-performance コマンドの1名のレビュー担当です。レビューのみ行い、ファイルは編集しないでください。
...
If both subagents return no findings, say `レンダリング性能・読み込み/計算コストの観点では指摘はありません`.

...
description: PHP / Laravel の差分を、インジェクション・マスアサインメント・XSS と、認可・マルチテナント分離・機微情報漏洩・入力検証の観点でセキュリティレビューするとき。「セキュリティレビュー（PHP）」「認可漏れチェック」「SQLi/XSS を見て」等で発動。コードは修正せず指摘のみ報告する。
allowed-tools: [Bash, Read, Grep, Glob, Agent]
---

# /review-php-security

Review PHP / Laravel changes against `<base>...HEAD` using two independent security-focused subagents. This is review-only: do not modify files.
...
In scope:
- インジェクション（SQLi）・マスアサインメント・XSS（Blade / フロント渡し HTML）
- 認可漏れ（Policy / Gate / Middleware）・マルチテナント分離・機微情報漏洩・入力検証

Out of scope:
- code edits
- レイヤー責務・命名・型安全・DB スキーマ・テスト: 各専用スキルを使う
...
```text
あなたは /review-php-security コマンドの1名のレビュー担当です。レビューのみ行い、ファイルは編集しないでください。
...
If all subagents return no findings, say `セキュリティ観点では指摘はありません`.

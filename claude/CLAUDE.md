...
このマシンのシェルはモダン CLI に置き換えてある。エージェント（Claude Code / Cursor / Codex 等）はコマンド実行時、**原則これらのモダン CLI を使う**。**`zsh/.zshrc` のエイリアス定義が正本**であり、そこで設定されているものは必ずそのツールを利用する。

置き換えマップ（`.zshrc` 準拠・該当コマンドが存在する場合のみ有効）:
...
- **モダン CLI は旧ツールと構文・出力が異なる**（`sd` は `sed` と置換構文が違う／`bat` はページャ・装飾が付く／`fd` は `find` と引数体系が違う）。各ツールの**ネイティブ構文**で正しく使い、GNU/BSD の構文をそのまま渡さない
- 上記は対話シェルのエイリアスのため、`cat`/`find`/`sed` 等はエイリアス経由でモダン CLI に解決される。**スクリプトや機械可読出力で GNU/POSIX の決定的な挙動が必要な場合のみ**、`command sed` のように実体を明示して呼ぶ（例: ページャを避けるなら `bat --paging=never`、素の連結なら `command cat`）
- 破壊系の確認エイリアス（`rm -i` / `cp -i` / `mv -i`）も尊重する。非対話で確認を避けたい場合は意図を明示してフラグや実体で対応する
...
- [ ] コメントは How/What（コード・命名・テストで表現すべき内容）でなく Why not（却下した代替案・トレードオフ・落とし穴）を説明しているか（→ 情報配置の四分割原則）
